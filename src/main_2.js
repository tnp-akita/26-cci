...
const game = {
  ...
  ball: { x: 0, y: 0, vx: 0, vy: 0, radius: 7, launched: false, speed: 198 },
  bullets: [],
  blocks: [],
  ammoInventory: 0,
  running: false,
...
const buildBlocks = () => {
  ...
  for (let row = 0; row < 6; row += 1) {
    for (let col = 0; col < 3; col += 1) {
      ...
      blocks.push(makeBlock(leftStart + col * columnStep, top + row * (blockHeight + gap), blockWidth, blockHeight, type));
    }

    for (let col = 0; col < 3; col += 1) {
      let type = "normal";
      if (row === 2 && col === 0) type = "special";
      blocks.push(makeBlock(leftStart + (col + 4) * columnStep, top + row * (blockHeight + gap), blockWidth, blockHeight, type));
    }
  }

  for (let row = 0; row < 3; row += 1) {
    blocks.push(makeBlock(leftStart + 3 * columnStep, 136 + row * (blockHeight + gap), blockWidth, blockHeight, "hard"));
  }

  for (let col = 0; col < 7; col += 1) {
    blocks.push(makeBlock(col * (46 + gap), 210, 46, 18, col >= 2 && col <= 4 ? "hard" : "normal"));
  }
...
const updateBall = (dt) => {
  ...
  if (
    ...
    const hitPoint = (ball.x - (game.paddle.x + game.paddle.width / 2)) / (game.paddle.width / 2);
...
const drawBlocks = () => {
  game.blocks.forEach((block) => {
    let color = "#bfd3bd";
    if (block.type === "hard") color = block.hitPoints === 2 ? "#7da6b5" : "#99bcc8";
...
const drawBullets = () => {
  game.bullets.forEach((bullet) => {
    if (game.ammoImageReady) {
      ctx.drawImage(game.ammoImage, bullet.x, bullet.y, bullet.width, bullet.height);
    } else {
...
const drawInventory = () => {
  ...
  ctx.textAlign = "right";
  ctx.fillText(game.infiniteAmmo ? "INF" : `x${game.ammoInventory}`, game.width - 6, game.height - 10);
};
...
function gameLoop(timestamp) {
  ...
  const dt = Math.min((timestamp - game.lastFrame) / 1000, 0.02);
  game.lastFrame = timestamp;
...
canvas.addEventListener("click", (event) => {
  ...
  if (x >= game.width - 42 && x <= game.width && y >= game.height - 102 && y <= game.height) {
